...
fn create_tpm_ek_ec384() -> Result<EndorsementKey, SvsmError> {
...
pub fn provision_ca_cert(cert: &[u8]) -> Result<(), SvsmError> {
    write_cert_nv(TPM2_NV_INDEX_VTPM_CA_CERT_CHAIN, cert)
}

fn write_cert_nv(nv_index: u32, cert: &[u8]) -> Result<(), SvsmError> {
